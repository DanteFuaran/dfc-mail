...
# ═══════════════════════════════════════════════
# ПРОВЕРКА УДАЛЁННОЙ ВЕРСИИ
# ═══════════════════════════════════════════════
check_remote_version() {
    curl -fsSL --connect-timeout 3 \
        "https://raw.githubusercontent.com/DanteFuaran/dfc-mail/main/version" 2>/dev/null \
        | grep '^version:' | cut -d: -f2 | tr -d ' \n'
}

version_gt() {
    # возвращает 0, если $1 > $2 (semver)
...
show_full_menu() {
    while true; do
        ...
        local remote_ver
        remote_ver=$(check_remote_version 2>/dev/null || echo "")

        local menu_title="    📧 DFC MAIL BOT v${ver}  |  ${status_text}"
        if [ -n "$remote_ver" ] && version_gt "$remote_ver" "$ver"; then
            menu_title="${menu_title}\n  ⬆️  Доступно обновление до v${remote_ver}"
        fi
